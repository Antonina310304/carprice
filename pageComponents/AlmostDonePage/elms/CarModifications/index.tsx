...
const CarModifications: NextPage<ICarModifications> = function ({ className }) {
  const dispatch = useDispatch();
  const { modificationId } = useSelector((state: IState) => state.carData.carDetail);

  const { modifications, statusModifications } = useSelector((state: IState) => state.catalogs);
